...
plt.plot(sol1.y[0], sol1.y[1], color = "b",label = "Solution for f")
plt.plot(sol2.y[0], sol2.y[1], color = "r", label = "Equilibrium position no. 1")
plt.plot(sol3.y[0], sol3.y[1], color = "g", label = "Equilibrium position no. 2")
plt.xlabel("$x$")
plt.ylabel("$\dot{x}$")
plt.title("f=" + str(f))
plt.legend()
plt.savefig(os.path.join(RESULTS_DIR, "lorenz_xz.png"))
plt.close()
...
plt.ylabel("$v(t)$")
plt.xlabel("$t$")
plt.savefig(os.path.join(RESULTS_DIR, "lorenz_x.png"))
plt.close()